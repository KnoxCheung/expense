...
async function fetchExpenses() {
  ...
  try {
    ...
    showNotification(
      `Error fetching data: ${error.message}. Check the console for more details.`,
      "error",
    );
  } finally {
...
async function updateBudgetStatus() {
  ...
  try {
    ...
    totalStatusDiv.innerHTML =
      "<h3 class='text-xl font-semibold mb-4'>Monthly Budget Status</h3>";
...
async function updateExpenseSummary() {
  try {
    ...
    showNotification(
      "Error fetching expense summary. Please try again.",
      "error",
    );
  }
}
...
document
  .getElementById("updateBudgetsBtn")
  .addEventListener("click", updateBudgets);
