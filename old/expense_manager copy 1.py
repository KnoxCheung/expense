...
class ExpenseManager:
    ...
    def get_expenses_for_range(self, days):
    ...
    def get_total_spent_in_range(self, days):
        expenses_in_range = self.get_expenses_for_range(days)
        return sum(expense.amount for expense in expenses_in_range)
...
if __name__ == "__main__":
    ...
    recent_expenses = manager.get_expenses_for_range(7)
    for expense in recent_expenses:
        print(f"{expense.date}: {expense.category} - ${expense.amount}")

    total_spent = manager.get_total_spent_in_range(7)
    print(f"\nTotal spent in the last 7 days: ${total_spent:.2f}")
